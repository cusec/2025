...
const SpeakerCard = ({ speaker }: { speaker: Speaker }) => {
  ...
  return (
    <div className="mb-8 bg-white rounded-lg shadow-md p-12 relative">
      ...
      {showMore && (
        <div
          className={`fixed top-0 left-0 w-full h-full bg-gray-500 bg-opacity-75 flex justify-center items-center z-50 transition duration-300 ease-in-out ${
            showMore ? "opacity-100 animate-fade-in" : "opacity-0"
          }`}
        >
          <div className="bg-white rounded-lg shadow-md p-12 relative w-11/12 md:w-7/10 lg:w-4/5 xl:w-3/4 2xl:w-2/3 mx-auto z-50 max-h-full overflow-y-auto">
            <button
            ...
            <h2 className="text-3xl font-bold mb-4 RobotoText">
              {speaker.name}
            </h2>
